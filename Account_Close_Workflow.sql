
/* =========================================================
FULL ACCOUNT CLOSE WORKFLOW - REFRACTORED WITH ACTIONTYPE
========================================================= */

/* 0. PREVIEW COPY OF ACTUAL TABLE */
IF OBJECT_ID('tempdb..#tmpB') IS NOT NULL DROP TABLE #tmpB;
SELECT * INTO #tmpB FROM AccountsB;

/* 1. BASE SNAPSHOT (FOR PRE/POST RECON) */
IF OBJECT_ID('tempdb..#BaseAccounts') IS NOT NULL DROP TABLE #BaseAccounts;

WITH Agg AS (
    SELECT
        acct_number,
        SUM(CASE WHEN acct_type = 1 THEN 1 ELSE 0 END) AS main_cnt,
        SUM(CASE WHEN acct_type = 1 AND status = 'ACTIVE' THEN 1 ELSE 0 END) AS main_active_cnt,
        SUM(CASE WHEN acct_type = 2 THEN 1 ELSE 0 END) AS sub_cnt,
        SUM(CASE WHEN acct_type = 2 AND status = 'ACTIVE' THEN 1 ELSE 0 END) AS sub_active_cnt
    FROM #tmpB
    GROUP BY acct_number
)
SELECT
    b.acct_id,
    b.acct_number,
    b.acct_type,
    b.status AS prev_status,
    NULL AS new_status,

    CASE
        WHEN a.main_active_cnt = 1 THEN 'ELIGIBLE_CLOSE'
        WHEN a.main_active_cnt = 0 AND a.sub_active_cnt > 0 THEN 'DO_NOT_CLOSE'
        WHEN a.main_cnt = 0 AND a.sub_active_cnt > 0 THEN 'DO_NOT_CLOSE'
        WHEN a.main_active_cnt = 0 AND a.sub_active_cnt = 0 THEN 'SKIP'
        ELSE 'SKIP'
    END AS eligibility,

    CASE
        WHEN a.main_active_cnt = 1 AND a.sub_cnt = 0 THEN 'CLOSE_MAIN'
        WHEN a.main_active_cnt = 1 AND a.sub_cnt > 0 AND a.sub_active_cnt = 0 THEN 'CLOSE_MAIN'
        WHEN a.main_active_cnt = 1 AND a.sub_active_cnt > 0 THEN 'CLOSE_SUB_AND_MAIN'
        WHEN a.main_active_cnt = 0 AND a.sub_active_cnt > 0 THEN 'INVESTIGATE'
        ELSE 'NA'
    END AS action_type,

    NULL AS status_flag,
    NULL AS status_details
INTO #BaseAccounts
FROM #tmpB b
JOIN Agg a
  ON a.acct_number = b.acct_number
JOIN StageAccountsA s
  ON s.acct_number = b.acct_number;

/* 2. Stage records not found in system → SKIP / NA */
INSERT INTO #BaseAccounts
(acct_id, acct_number, acct_type, eligibility, action_type, status_flag, status_details)
SELECT
    NULL,
    s.acct_number,
    NULL,
    'SKIP',
    'NA',
    'FAIL',
    'Account not found in system'
FROM StageAccountsA s
WHERE NOT EXISTS (
    SELECT 1 FROM #tmpB b WHERE b.acct_number = s.acct_number
);

/* 3. Per-account transactional enforcement */
DECLARE @AcctNumber varchar(50);

DECLARE c CURSOR LOCAL FAST_FORWARD FOR
SELECT DISTINCT acct_number
FROM #BaseAccounts
WHERE eligibility = 'ELIGIBLE_CLOSE';

OPEN c;
FETCH NEXT FROM c INTO @AcctNumber;

WHILE @@FETCH_STATUS = 0
BEGIN
    BEGIN TRY
        BEGIN TRAN;

        IF EXISTS (
            SELECT 1 FROM #BaseAccounts
            WHERE acct_number = @AcctNumber
              AND action_type = 'CLOSE_SUB_AND_MAIN'
        )
        BEGIN
            UPDATE t
            SET status = 'CLOSED', closed_dt = SYSDATETIME()
            FROM #tmpB t
            JOIN #BaseAccounts b ON b.acct_id = t.acct_id
            WHERE b.acct_number = @AcctNumber
              AND b.acct_type = 2
              AND t.status = 'ACTIVE';

            IF EXISTS (
                SELECT 1 FROM #tmpB
                WHERE acct_number = @AcctNumber
                  AND acct_type = 2
                  AND status = 'ACTIVE'
            )
                THROW 70001, 'Sub closure failed', 1;
        END

        UPDATE t
        SET status = 'CLOSED', closed_dt = SYSDATETIME()
        FROM #tmpB t
        JOIN #BaseAccounts b ON b.acct_id = t.acct_id
        WHERE b.acct_number = @AcctNumber
          AND b.acct_type = 1
          AND t.status = 'ACTIVE';

        IF EXISTS (
            SELECT 1 FROM #tmpB
            WHERE acct_number = @AcctNumber
              AND acct_type = 1
              AND status = 'ACTIVE'
        )
            THROW 70002, 'Main closure failed', 1;

        COMMIT;

        UPDATE b
        SET
            b.new_status = t.status,
            b.status_flag = 'SUCCESS',
            b.status_details = action_type
        FROM #BaseAccounts b
        JOIN #tmpB t ON t.acct_id = b.acct_id
        WHERE b.acct_number = @AcctNumber;

    END TRY
    BEGIN CATCH
        IF @@TRANCOUNT > 0 ROLLBACK;

        UPDATE #BaseAccounts
        SET status_flag = 'FAIL', status_details = ERROR_MESSAGE()
        WHERE acct_number = @AcctNumber;
    END CATCH;

    FETCH NEXT FROM c INTO @AcctNumber;
END;

CLOSE c;
DEALLOCATE c;

/* 5. Assign FAIL for DO_NOT_CLOSE / SKIP */
UPDATE #BaseAccounts
SET status_flag = 'FAIL',
    status_details = action_type
WHERE eligibility IN ('DO_NOT_CLOSE','SKIP')
  AND status_flag IS NULL;

/* 6. Preview output */
SELECT *
FROM #BaseAccounts
ORDER BY acct_number, acct_type;

/* 7. Apply to production */
UPDATE p
SET p.status = t.status,
    p.closed_dt = t.closed_dt
FROM AccountsB p
JOIN #tmpB t
  ON t.acct_id = p.acct_id
WHERE p.status <> t.status;
